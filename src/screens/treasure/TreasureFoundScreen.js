...
export default function TreasureFoundScreen({
  ...
  }, [isComplete, onCompleteToHome, onContinue, onMenu]);

  const rewardRows = [
    { label: "XP for funnet", value: `+${xpRule.xpPerTreasure}` },
    { label: "Skatter", value: `${found}/${total}` },
    { label: "Status", value: isComplete ? "Alle funnet" : "Jakten fortsetter" }
  ];
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container}>
        <TouchableOpacity
          ...
          accessibilityLabel="Tilbake til jakten"
        >
          <Text style={styles.backButtonText}>Tilbake til jakten</Text>
        </TouchableOpacity>
        ...
        <Text style={styles.title}>{isComplete ? "Alle skattene er funnet" : "Skatten er funnet"}</Text>
        <Text style={styles.body}>
          {isComplete
            ? "Jakten er fullført. Resultatet og XP-en er klare."
            : `${total - found} ${total - found === 1 ? "skatt gjenstår" : "skatter gjenstår"}.`}
        </Text>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.cardText}>
            {isComplete
              ? "Siste skatt er registrert. Resultatet vises nå."
        ...
        <View style={styles.card}>
          ...
          {rewardRows.map((row, index) => (
            <View
              key={row.label}
              style={[styles.rewardRow, index === rewardRows.length - 1 && styles.rewardRowLast]}
            >
        ...
        <TouchableOpacity
          ...
          onPress={leaveToMenu}
          accessibilityRole="button"
          accessibilityLabel="Avslutt og gå til meny"
        >
          <Text style={styles.secondaryButtonText}>Avslutt og gå til meny</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  backButton: { minHeight: 44, alignSelf: "flex-start", justifyContent: "center", paddingHorizontal: 8, marginBottom: 16 },
  backButtonText: { color: "#F59E0B", fontSize: 16, fontWeight: "700" },
  ...
  successMark: { width: 28, height: 28, borderRadius: 14, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(34, 197, 94, 0.14)", borderWidth: 1, borderColor: "rgba(34, 197, 94, 0.3)", marginRight: 10 },
  successMarkText: { color: "#22C55E", fontSize: 16, fontWeight: "900", lineHeight: 18 },
  sectionTitle: { color: "#E2E8F0", fontSize: 16, fontWeight: "800" },
  cardText: { color: "#E2E8F0", fontSize: 15, lineHeight: 22 },
  rewardRow: { minHeight: 44, flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderBottomWidth: 1, borderBottomColor: "rgba(148, 163, 184, 0.12)" },
  rewardRowLast: { borderBottomWidth: 0 },
  rewardLabel: { color: "#94A3B8", fontSize: 15, fontWeight: "600" },
  rewardValue: { color: "#E2E8F0", fontSize: 15, fontWeight: "700" },
