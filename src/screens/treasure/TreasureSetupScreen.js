...
function VariantGraphic({ sonar }) {
  return (
    <View style={[s.graphic, sonar ? s.sonarGraphic : s.fogGraphic]}>
      ...
      {sonar ? <View style={s.sonarPing} /> : null}
  ...
  return (
    <SafeAreaView edges={["left", "right", "bottom"]} style={s.safe}>
      <KeyboardAvoidingView style={s.safe} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} bounces={false} alwaysBounceVertical={false} overScrollMode="never">
          <View style={s.frame}>
...
const s = {
  ...
  sonarPressed: { transform: [{ scale: 0.985 }], shadowColor: C.purple, shadowOpacity: 0.4, shadowRadius: 12 },
  ...
  sonarPing: { position: "absolute", top: 21, right: 21, width: 15, height: 15, borderRadius: 8, backgroundColor: "#22D3EE" },
  ...
  diffTitle: { color: C.text, fontSize: 14, lineHeight: 17, marginTop: 0 },
  diffSubtitle: { color: C.muted, fontSize: 12, lineHeight: 15, marginTop: 0 },
